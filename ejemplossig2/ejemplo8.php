<?php
	...
	$layerPoligono = ms_newLayerObj($mapObject);
	$layerPoligono->set( "name", "bufferrio");
	$layerPoligono->set( "type", MS_LAYER_POLYGON);
	$layerPoligono->set( "status", MS_ON);
	$layerPoligono->set("connectiontype",MS_POSTGIS);
	$layerPoligono->set("connection",$conexion_basedatos);
	$layerPoligono ->set("data","the_geom from (select b.the_geom, b.gid, buffer(a.the_geom, 5) from rios as a, lotes as b where intersects (buffer(a.the_geom,5) , b.the_geom )) as mzyum using unique gid using srid=-1");
	$clasePoligono = ms_newClassObj($layerPoligono);
	$estiloPoligono = ms_newStyleObj($clasePoligono);
	$estiloPoligono->color->setRGB(65 ,0 ,24);
	$estiloPoligono->outlinecolor->setRGB(0 ,0, 245);
	...
	$layerPoligono->set("connectiontype",MS_POSTGIS);
	$layerPoligono->set("connection",$conexion_basedatos);
	$layerPoligono ->set("data","the_geom from (select gid, buffer(the_geom, 500) as the_geom  from pluviometros) as mzyum using unique gid using srid=-1");
	$clasePoligono = ms_newClassObj($layerPoligono);
	$estiloPoligono = ms_newStyleObj($clasePoligono);
	$estiloPoligono->color->setRGB(0 ,0 ,255);
	$estiloPoligono->outlinecolor->setRGB(0 ,0, 0);
...
<html>
	...
	<head>
		<title>Ejemplo 8 de Archivo que no usa .MAP</title>
	</head>
